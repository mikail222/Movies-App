import React from "react";
import { AiOutlineMenu } from "react-icons/ai";
import { MdKeyboardVoice } from "react-icons/md";
import { TfiSearch } from "react-icons/tfi";
import { IoIosNotificationsOutline } from "react-icons/io";
import { BiVideoPlus } from "react-icons/bi";

const Header = () => {
  return (
    <div className="flex flex-row justify-center items-center fixed w-[100%]">
      <div className="flex flex-row justify-center items-center w-[80%]">
        <div className="flex flex-row justify-center items-center ml-[1.5rem]  ">
          <div className="voice  w-[2.5rem] h-[38px] mr-[1rem] rounded-[80%] flex flex-col justify-center items-center cursor-pointer">
            <AiOutlineMenu className="w-[25px] h-[20px]  " />
          </div>
          <div className="flex flex-row">
            <img
              className="w-[110px] h-[55px] items-center"
              src="data:image/png;base64,"
              alt="play"
            />
            <p className="text-[0.65rem] text-[gray]  font-thin font-sans">
              NG
            </p>
          </div>
        </div>

        <form className="flex flex-row w-[80%] h-[6.5vh]">
          <input
            type="search"
            name="searchMovies"
            placeholder="search"
            style={{
              border: "1px solid  lightgray",
            }}
            className="pl-[1rem] w-[41vw]  ml-[12.5rem]"
          />
          <div className="search">
            <TfiSearch className="mt-[12px] w-[50px]  " />
          </div>
        </form>

        <div className="voice  w-[2.5rem] h-[38px] bg-[#e5e7eb]  mr-[1rem] rounded-[80%] flex flex-col justify-center items-center">
          <MdKeyboardVoice className="w-[25px] h-[35px]" />
        </div>
      </div>
      <div className="flex flex-row justify-center items-center ml-[1rem] w-[20%] gap-[2rem]">
        <BiVideoPlus className="w-[25px] h-[25px]" />
        <IoIosNotificationsOutline className="w-[25px] h-[25px]" />
      </div>
    </div>
  );
};

export default Header;
